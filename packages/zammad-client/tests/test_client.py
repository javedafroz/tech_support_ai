...
@respx.mock
@pytest.mark.asyncio
async def test_create_ticket_serializes_attachment_mime_type_alias(
    client: ZammadClient, zammad_base: str
):
    route = respx.post(f"{zammad_base}/api/v1/tickets").mock(
        return_value=httpx.Response(
            201,
            json={"id": 22, "number": "22022", "title": "Blue screen"},
        )
    )
    ...
    assert '"mime-type":"image/png"' in payload
