...
class WorkflowEngine:
    ...
    def build_command(self, intent: StructuredIntent, user: UserContext) -> TicketCommand:
        ...
        if intent.intent == IntentName.ADD_ATTACHMENT:
            return self._add_attachment_command(intent, user)
    ...
    def _add_attachment_command(self, intent: StructuredIntent, user: UserContext) -> TicketCommand:
        payload = intent.payload
        ticket_number = str(payload.get("ticket_number") or "").strip()
        if not ticket_number:
            raise ValueError("ticket_number is required for AddAttachment")

        note = (payload.get("note") or "Attachment added via Tech Support AI chat.").strip()
        return TicketCommand(
            type=TicketCommandType.ADD_ATTACHMENT,
            session_id=intent.session_id,
            user_id=intent.user_id,
            payload={
