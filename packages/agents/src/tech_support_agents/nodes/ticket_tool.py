...
async def ticket_tool_node(state: SupportGraphState) -> dict:
    ...
    elif command.type == TicketCommandType.ADD_ATTACHMENT:
        statuses.append("Uploading attachment…")
    else:
        return {"error": f"Unsupported ticket command: {command.type}"}
    ...
    try:
        ...
        if command.type == TicketCommandType.ADD_ATTACHMENT:
            if result.ticket is None:
                return {
                    "error": "Ticket provider response missing ticket for attachment",
                    "system_statuses": statuses,
                    "assistant_reply": "Could not attach the file due to an invalid provider response.",
                    "needs_clarification": True,
                }
            ticket_number = result.ticket.display_number
            filenames = [
            ...
            files_label = ", ".join(filenames) if filenames else "your file(s)"
            return {
                "provider_response": response,
                "active_ticket_number": ticket_number,
                "system_statuses": statuses,
                "assistant_reply": (
                    f"I've attached {files_label} to support ticket #{ticket_number}."
                ),
            }
