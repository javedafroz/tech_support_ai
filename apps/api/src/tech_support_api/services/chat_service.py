...
from tech_support_api.db.models import ChatMessage, ChatSession, KbDeflectionEvent
...
class GraphExecutionResult:
    ...
    troubleshoot_state: dict | None = None
...
class ChatService:
    ...
    async def send_message_stream(
        ...
        attachment_rows = await self._attachments.link_to_message(
        ...
        if attachment_rows:
            user_message.attachments = self._attachments.to_message_refs(attachment_rows)
        ...
        try:
            ...
            async for event_type, payload in self._stream_graph_execution(
                ...
                history=history,
                pending_attachments=self._attachments.to_graph_payload(attachment_rows),
                active_ticket_number=session.active_ticket_number,
                troubleshoot_state=context.troubleshoot if context else None,
            ):
        ...
        *,
        ...
        if is_graph_enabled():
            graph_turn = await get_graph_runner().invoke_turn(
                ...
                active_ticket_number=active_ticket_number or session.active_ticket_number,
                kb_rag_enabled=get_settings().kb_rag_enabled,
                troubleshoot_state=troubleshoot_state,
            )
            ...
            return GraphExecutionResult(
                ...
                troubleshoot_state=graph_turn.troubleshoot_state,
                deflection=graph_turn.deflection,
            )
    ...
    async def _finalize_message_exchange(
        ...
        *,
        ...
        self._record_deflection_event(session_id, graph_result.deflection)
        ...
        await self._redis.record_turn(
            ...
            troubleshoot=graph_result.troubleshoot_state,
        )
        await self._redis.record_turn(
            session_id,
            role="assistant",
    ...
    def _record_deflection_event(self, session_id: UUID, deflection: dict | None) -> None:
        if not deflection:
            return
        document_id: UUID | None = None
        raw_document_id = deflection.get("document_id")
        if raw_document_id:
            try:
        ...
        self._db.add(
            KbDeflectionEvent(
                ...
                outcome=str(deflection.get("outcome", "unknown"))[:32],
                steps_count=int(deflection.get("steps_count", 0)),
            )
        )
