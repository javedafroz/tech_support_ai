.PHONY: up up-kb up-all down migrate seed-kb api web admin install test lint check-venv e2e-install e2e e2e-ui test-live test-live-ui test-live-install docs docs-serve docs-build
...
up:
	docker compose up -d postgres redis minio minio-init qdrant keycloak
...
seed-kb: check-venv
	$(VENV_BIN)/python scripts/seed_kb.py
...
docs-serve: check-venv
	$(VENV_BIN)/mkdocs serve -a 127.0.0.1:8088
